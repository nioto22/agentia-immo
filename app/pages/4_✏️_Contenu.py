...
# DB fallback for calendar
if not calendar_content:
    try:
        from db import get_latest_calendar
        cal_db = get_latest_calendar()
        if cal_db:
            calendar_content = cal_db["calendar_content"]
...
if calendar_content:
    ...
    st.info("Aucun calendrier editorial en session.")
    st.page_link("pages/3_\U0001f4c5_Calendrier.py", label="Generer votre calendrier d'abord", icon="\U0001f4c5")
...
dim_key = (platform, content_format)
if dim_key in FORMAT_DIMENSIONS:
    st.caption(f"Dimensions recommandees : {FORMAT_DIMENSIONS[dim_key]}")
...
"""
    ...
    save_post(profile_id, platform, content_format, topic.strip(), response, model_label)
    st.rerun()
...
if st.session_state.generated_posts:
    ...
    for i, post in enumerate(st.session_state.generated_posts):
        ...
        with st.expander(f"Voir le contenu complet", expanded=(i == 0)):
            ...
            tab_labels = ["Instagram", "LinkedIn", "Facebook"]
            # Reorder so the post's platform is first
            post_platform = post["platform"]
            if post_platform in tab_labels:
                tab_labels = [post_platform] + [p for p in tab_labels if p != post_platform]

            tabs = st.tabs(tab_labels)

            # Parse once for all tabs
            parsed = parse_generated_content(post["content"])
            copy_text = parsed["caption"]
            if parsed["hashtags"]:
                copy_text += "\n\n" + parsed["hashtags"]

            # Height varies by platform (Instagram 4:5 image is taller)
            preview_heights = {"Instagram": 820, "LinkedIn": 620, "Facebook": 600}

            for tab_idx, tab_platform in enumerate(tab_labels):
                with tabs[tab_idx]:
                    preview_html = render_social_preview(tab_platform, post["content"], agent_name)
                    h = preview_heights.get(tab_platform, 650)
                    components.html(preview_html, height=h, scrolling=True)

            # Copyable text below tabs
            st.markdown("**Texte pret a copier**")
            st.code(copy_text, language=None)

            if parsed["cta"]:
                st.caption(f"CTA : {parsed['cta']}")
            if parsed["visual_suggestion"]:
